...
class GitHub:
    def __init__(
        ...
        self.remote = remote.rstrip("/").replace(
        ...
        if username and token:
            self.headers = {
                ...
                % base64.b64encode(b"%s:%s" % (username, token))
            }
        else:
            self.headers = {}
    ...
    def sha(self):
        result = self.requests.get(
            "%s/commits?per_page=1&sha=%s" % (self.remote, self.branch),
            logger=self.logger,
        ...
        if result.status_code == 200:
            ...
            raise Exception(
                "Unexpected response from GitHub: %d:%s"
    ...
    def download(self, sha=None, destination=None, currentDir="", base=""):
        fileList = self.requests.get(
            "%s/contents/%s?ref=%s"
            % (self.remote, self.io.path(base, currentDir), sha),
            logger=self.logger,
            headers=self.headers,
        )
        ...
        for file in fileList.json():
            if file["type"] == "file":
                result = self.requests.get(
                    file["download_url"], logger=self.logger, headers=self.headers
                )
                ...
                self.download(
                    ...
                    currentDir=self.io.path(currentDir, file["name"]),
                    base=base,
                )
