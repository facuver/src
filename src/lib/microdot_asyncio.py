...
class Request(BaseRequest):
    ...
    async def create(app, client_stream, client_addr):
        ...
        # body
        body = await client_stream.read(content_length) if content_length else b""

        return Request(app, client_addr, method, url, http_version, headers, body)
...
class Microdot(BaseMicrodot):
    ...
    async def dispatch_request(self, reader, writer):
        req = await Request.create(self, reader, writer.get_extra_info("peername"))
        if req:
            ...
            try:
                ...
                if res is None:
                    if 500 in self.error_handlers:
                        res = await self._invoke_handler(self.error_handlers[500], req)
                    else:
